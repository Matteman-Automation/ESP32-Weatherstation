...
void setup() {
  ...
  Serial.println("Sensor 7700 gestart.");
}
...
// --------------------------------------------------
// Correctie luchtdruk naar druk op zee niveau
// --------------------------------------------------
  ...
  if (lNowMS - lSHT41_MS >= lSHT41_Interval) {    // Iedere 5 seconden Temp & Vocht doorgeven
    ...
    if (!sht4.getEvent(&humidity, &temp)) {
      ...
      if (iWiFiTry < 11) { 
        ...
        MQTTclient.publish("sensor/weerstation/licht", String(fLux).c_str());
        Serial.print("sensor/weerstation/licht = "); Serial.print(fLux); Serial.println(" lux");
